...
impl JuliaEngine {
    pub fn new() -> Self {
        Self {
            executable: resolve_julia_binary(),
        }
    }
    ...
    fn ensure_executable(&self) -> Result<&Path> {
        self.executable.as_deref().ok_or_else(|| {
            anyhow::anyhow!(
                "Julia support requires the `julia` executable. Install Julia from https://julialang.org/downloads/ and ensure `julia` is on your PATH."
            )
        })
    }
    ...
    fn write_temp_source(&self, code: &str) -> Result<(TempDir, PathBuf)> {
        let dir = Builder::new()
            .prefix("run-julia")
            .tempdir()
            .context("failed to create temporary directory for Julia source")?;
        let path = dir.path().join("snippet.jl");
        ...
        fs::write(&path, contents).with_context(|| {
            format!(
                "failed to write temporary Julia source to {}",
                path.display()
            )
        })?;
        Ok((dir, path))
...
impl LanguageEngine for JuliaEngine {
    ...
    fn start_session(&self) -> Result<Box<dyn LanguageSession>> {
        ...
        Ok(Box::new(JuliaSession::new(executable)?))
    }
}

fn resolve_julia_binary() -> Option<PathBuf> {
    which::which("julia").ok()
}

#[derive(Default)]
struct JuliaSessionState {
...
impl JuliaSession {
    fn new(executable: PathBuf) -> Result<Self> {
        let workspace = Builder::new()
            .prefix("run-julia-repl")
            .tempdir()
            .context("failed to create temporary directory for Julia repl")?;
        let session = Self {
            executable,
            workspace,
            state: JuliaSessionState::default(),
    ...
    fn source_path(&self) -> PathBuf {
        self.workspace.path().join("session.jl")
    }

    fn persist_source(&self) -> Result<()> {
        let source = self.render_source();
        fs::write(self.source_path(), source)
            .with_context(|| "failed to write Julia session source".to_string())
    }
    ...
    fn run_program(&self) -> Result<std::process::Output> {
        ...
        cmd.output().with_context(|| {
            format!(
                "failed to execute {} for Julia session",
    ...
    fn apply_import(&mut self, code: &str) -> Result<(ExecutionOutcome, bool)> {
        ...
        if !success {
            for line in inserted {
                self.state.imports.remove(&line);
            }
            self.persist_source()?;
        }
        Ok((outcome, success))
...
impl LanguageSession for JuliaSession {
...
fn classify_snippet(code: &str) -> JuliaSnippet {
    if is_import(code) {
        return JuliaSnippet::Import;
    }

    if is_declaration(code) {
        return JuliaSnippet::Declaration;
    }

    if should_wrap_expression(code) {
        return JuliaSnippet::Expression;
    }
...
fn is_import(code: &str) -> bool {
    code.lines().all(|line| {
        ...
        trimmed.starts_with("using ") || trimmed.starts_with("import ")
    })
}
